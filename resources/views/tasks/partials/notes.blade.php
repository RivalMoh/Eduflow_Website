            ...
            @forelse($notes->take(5) as $note)
                <div class="bg-white border border-gray-200 rounded-lg p-4 shadow-sm hover:shadow transition-shadow">
                    <div class="flex justify-between items-start">
                        <h3 class="font-medium text-gray-800 truncate pr-2">{{ $note->title }}</h3>
                        @if($note->tag)
                        ...
                        @foreach($notes as $note)
                            <div class="bg-white border border-gray-200 rounded-lg p-6 shadow-sm hover:shadow-md transition-shadow h-fit">
                                <div class="flex justify-between items-start mb-3">
                                    <h3 class="font-medium text-gray-800 text-lg">{{ $note->title }}</h3>
                                    <div class="relative" x-data="{ open: false }">
                                        ...
                                        <div x-show="open" @click.away="open = false" class="absolute right-0 mt-1 w-32 bg-white rounded-md shadow-lg py-1 z-10 border border-gray-200" style="display: none;">
                                            <a href="#" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 edit-note" data-note-id="{{ $note->id }}">
                                                <i class="fas fa-edit mr-2"></i>Edit
                                            </a>
                                            <a href="#" class="block px-4 py-2 text-sm text-red-600 hover:bg-gray-100 delete-note" data-note-id="{{ $note->id }}">
                                ...
                                <div class="pt-3 border-t border-gray-100 text-sm text-gray-500">
                                    <div>Created {{ $note->created_at->diffForHumans() }}</div>
                                    @if($note->updated_at->ne($note->created_at))
                                        <div class="text-xs text-gray-400">Last edited {{ $note->updated_at->diffForHumans() }}</div>
                                    @endif
                                </div>
                            </div>
                        @endforeach

<div class="flex items-center justify-between h-16 px-6">
    ...
    <div class="flex items-center">
        <a href="{{ route('home') }}" class="flex items-center">
            <img src="{{ asset('images/icons/EduFlow-text-black-svg.png') }}" alt="X" class="h-8 w-auto">
        </a>
    </div>
        ...
        <div class="flex items-center space-x-4">
                    ...
                    >
                        <div class="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 font-medium overflow-hidden flex-shrink-0">
                            @if(auth()->user()->profile_photo_path)
                                <img src="{{ auth()->user()->profile_photo_path }}" alt="{{ auth()->user()->name }}" class="w-full h-full object-cover">
                            @else
            ...
            @else
                ...
                <div class="flex items-center space-x-2">
                    <a href="{{ route('login') }}" class="px-3 py-1.5 text-sm font-medium text-gray-700 hover:text-indigo-600">
                        {{ __('Log in') }}
                    </a>
                    <a href="{{ route('register') }}" class="px-3 py-1.5 text-sm font-medium text-white bg-indigo-600 rounded-md hover:bg-indigo-700">
                        {{ __('Sign up') }}
                    </a>
                </div>
            @endauth

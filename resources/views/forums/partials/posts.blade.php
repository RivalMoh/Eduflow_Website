    ...
    <div class="p-5">
        ...
        <div class="mb-4">
            <p class="text-gray-800 mb-3">
                This is a sample forum post #{{ $i }}. Here's some content to demonstrate how a post might look in the forum.
                @if($i % 2 === 0)
        ...
        <div x-show="showComments" x-transition class="space-y-3 mt-4 pt-4 border-t border-gray-100">
            ...
            <div class="flex items-start space-x-2">
                <div class="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center text-gray-600 text-xs font-medium flex-shrink-0">
                    {{ strtoupper(substr("User $j", 0, 1)) }}
                </div>
                <div class="flex-1 bg-gray-50 rounded-lg p-3">
                    <div class="flex items-center justify-between">
                        <span class="text-sm font-medium">User {{ $j }}</span>
                        <span class="text-xs text-gray-400">{{ now()->subMinutes(rand(5, 60))->diffForHumans() }}</span>
                    </div>
